...
- **HomeScreen "How it works"**: Light, ghost-style 2×2 tile grid with "How it works" title. Uses Pressable with subtle pressed state (#EEE9FF). Each tile: 48% width, 56px min height, 12px border radius. Ghost style: soft lavender background (#F7F5FF), 1px border (#E7E3FF), no shadow. Icons: Describe (create-outline), Scan (image-outline), Preview (sparkles-outline), Build (hammer from MaterialCommunityIcons). Text: 13px, 600 weight, dark color (#1B133C). Each tile deep-links to NewProject with focus/scroll to relevant section: Describe → desc input focus, Scan → photo picker, Preview → suggestions card scroll, Build → action buttons scroll. Spacing: 4px top, 12px bottom, 8px gap below title.
