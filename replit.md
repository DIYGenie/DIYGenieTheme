...
- **Build with AI Preview on NewProject**: Purple CTA button on NewProject screen that enables users to build projects with AI-generated visual previews. Flow: (1) ensures project exists via `ensureProjectForDraft()`, (2) requests preview via `requestProjectPreview()` (POST to DEV webhooks `/api/projects/:id/preview`), (3) shows success toast or friendly alert if unavailable, then (4) triggers plan build with `pollProjectReady()` which polls project status every 2 seconds with 60-second timeout, and (5) auto-navigates to ProjectDetails when plan is ready. Button disabled until form is valid (title ≥3, description ≥10, budget, skill level). **Graceful Error Handling**: Always navigates to ProjectDetails regardless of preview API response; treats 404/422 as soft success; never throws exceptions to prevent red error overlay in Expo Go. Uses `canSubmit` validation check with purple brand color (#7C3AED) and disabled state during loading. **Loading Banner**: When building (`isBuilding` state), displays purple-themed loading banner above CTA buttons with "Building your plan…" message, disables all inputs/buttons with `pointerEvents: 'none'` and 0.6 opacity, handles back navigation cleanup, and shows timeout alert after 60 seconds if plan doesn't complete.
...
- **Plan Waiting & Auto-Navigation**: All project card taps (Home and Projects list) check plan readiness via `fetchProjectPlanMarkdown()` before routing - if 409 (not ready), routes to PlanWaiting screen; if 200 (ready), routes directly to ProjectDetails. `PlanWaiting` screen has auto-polling (2-second intervals) that navigates to ProjectDetails when plan is ready. NewProject uses in-place loading banner instead of navigation - polls status with `pollProjectReady()` and auto-navigates to ProjectDetails when complete. ProjectDetails header badge shows "Plan ready" only when `planObj` exists (actual parsed data), not just status field.
