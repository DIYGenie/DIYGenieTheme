...
- **Build with AI Preview on NewProject**: Purple CTA button on NewProject screen that enables users to build projects with AI-generated visual previews. Flow: (1) ensures project exists via `ensureProjectForDraft()`, (2) requests preview via `requestProjectPreview()` (POST to `/api/projects/:id/preview`), (3) shows toast confirmation, then (4) navigates to Projects → ProjectDetails. Button disabled until form is valid (title ≥3, description ≥10, budget, skill level). Uses `canSubmit` validation check with purple brand color (#7C3AED) and disabled state during loading.
