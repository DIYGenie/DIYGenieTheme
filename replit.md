...
- **Project Details Screen**: Displays real-time project information including name, status, and latest scan image. Uses `fetchProjectById()` and `fetchLatestScanForProject()` API helpers to load data, with automatic refresh on focus via `useFocusEffect`. Shows project overview, materials, and steps when available. Falls back to "No scan image yet" placeholder when no room scans exist for the project. **Plan Output Display**: When project status is 'ready', automatically fetches and displays the build plan from `/api/projects/:id/plan` endpoint. Uses lightweight Markdown component (`app/components/Markdown.tsx`) to render plan content with heading levels (# ## ###), bullet lists, and paragraphs. **Smart Refresh**: The "Refresh" button intelligently builds the plan if not ready (POST `/api/projects/:id/build-without-preview`), then polls `/api/projects/:id/plan` until available (15s timeout with 900ms intervals). This eliminates "stuck on draft" issues by automatically triggering builds when needed. Shows helpful messages when plan is building or not yet available (409 status). Plan section is scrollable within the main ScrollView container.
