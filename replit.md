...
- **New Project Photo Section (`NewProjectMedia.js`)**: Displays two upload options - "Scan room" and "Upload Photo". Uploaded photos are tagged with `source: 'upload', allowMeasure: false` in V1, disabling measurement features for non-AR uploads. Saved scan preview card shows "Mark Area" button for all scans, but "Measure" button only appears for AR scans (`allowMeasure: true`). Help text guides users: "Measurements are available when you use Scan room." Event bridge system (`subscribeScanPhoto`/`emitScanPhoto`) enables seamless photo transfer from scan flow to project creation.
