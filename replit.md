...
- **Demo Project Feature**: "Try a Sample Project" CTA available on HomeScreen (above templates) and ProjectsScreen (empty state). Calls `/api/demo-project` endpoint via `createOrFetchDemoProject()` API function, creates or reuses user's demo project (marked `is_demo=true`, doesn't count against quotas), navigates to ProjectDetails. Includes robust error handling with try/catch blocks, console logging for debugging, navigation fallbacks, loading states with ActivityIndicator, and user-friendly error alerts.
