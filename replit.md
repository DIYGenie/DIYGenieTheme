...
- **New Project Screen (NewProject.tsx)**: Primary project creation interface with guided flow: Enter details → select photo → Design Suggestions (beta) → Edit build prompt → Generate Plan. Auto-creates draft project when form is valid (description ≥10 chars, budget, skill), keeps photo locally for preview. Image upload is skipped entirely to avoid 500 errors and keep flow cheap. Includes testIDs for QA: np-suggestions-card, np-suggestions-refresh, np-refine-input, np-refine-apply, np-refine-reset, np-generate-plan.
    - **Project Creation**: Always uses `user_id: 'auto'` which server maps to TEST_USER_ID. Handles `{ok:false, error/details}` responses as 422 validation errors, showing full server message in Alert. Stores returned `item.id` as `draftId`. Client-side validation ensures description ≥10 chars, budget, and skill level before API call.
    - **Design Suggestions (beta)**: Shows after photo selection with deterministic suggestions from POST /api/projects/:id/suggestions. Features inline loading ("Analyzing your photo…"), 4-5 selectable chip bullets, optional tags, and refresh button. Each suggestion can be selected via checkbox/chip tap. Empty state when no photo: "Add a room photo to get design suggestions." Never blocks form interaction. Caption under photo: "We'll upload your photo when you generate a preview."
    - **Prompt Refine Editor**: Users can select design suggestions and insert them into the build prompt. State management uses `promptText` (string) and `selectedTips` (Set<number>). Auto-prefills with base prompt: `Build plan for: "${description}". Budget: ${budget}. Skill: ${skillLevel}.` Features two action buttons:
        - **Insert selected into prompt** (testID: np-refine-apply): Appends selected suggestion bullets to promptText, disabled when no tips selected
        - **Reset prompt** (testID: np-refine-reset): Restores base prompt and clears selections
    - **Editable Build Prompt**: Multiline TextInput (testID: np-refine-input, minHeight: 120) bound to `promptText` allows full customization. Appears when form is valid. Users can manually edit or use suggestion insertion buttons.
    - **Generate Plan**: Calls POST /api/projects/:id/build-without-preview with `{user_id: 'auto', prompt: promptText}`. Handles `{ok:false}` responses with full error message display. Shows loading state ("Requesting…"), then navigates to Projects list on success.
