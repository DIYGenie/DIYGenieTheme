...
- **New Project Screen (NewProject.tsx)**: Primary project creation interface with guided flow: Enter details → select photo → Design Suggestions (beta) → Generate Plan. Includes testIDs for QA: np-suggestions-card, np-suggestions-refresh, np-apply-suggestions, np-generate-preview, np-build-no-preview.
