...
- **New Project Screen (NewProject.tsx)**: Primary project creation interface with guided flow: Enter details → select photo → Smart Suggestions (beta) → Edit build prompt → Generate Plan. Auto-creates draft project when form is valid (description ≥10 chars, budget, skill), keeps photo locally for preview. Image upload is skipped entirely (STUB_SKIP_IMAGE_UPLOAD flag) to avoid 500 errors and keep flow cheap. Includes testIDs for QA: np-suggestions-card, np-suggestions-refresh, np-generate-plan.
    ...
    - **Generate Plan**: Calls POST /api/projects/:id/build-without-preview with the edited prompt and user_id. Shows loading state ("Generating…"), then navigates to Projects list on success.
