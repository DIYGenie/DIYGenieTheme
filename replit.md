...
- **New Project Photo Section (`NewProjectMedia.js`)**: Displays two upload options - "Scan room" (AR, coming soon) and "Upload Photo" (gallery picker). Uploaded photos are tagged with `source: 'upload'` and show only the thumbnail preview. **AR-only tools**: "Adjust Area" button and RoiModal only appear for AR scans (`source: 'ar'`). For uploads, shows help text: "Tools are available when you use Scan room." MeasureModal completely removed - not mounted for uploads to prevent state glitches. Uses Alert for user feedback.
