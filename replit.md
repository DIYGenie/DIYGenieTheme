...
- **New Project Screen (NewProject.tsx)**: Primary project creation interface with guided flow: Enter details → select photo → Smart Suggestions (beta) → Edit build prompt → Generate Plan. Auto-creates draft project when form is valid (description ≥10 chars, budget, skill), keeps photo locally for preview. Image upload is skipped entirely to avoid 500 errors and keep flow cheap. Includes testIDs for QA: np-suggestions-card, np-suggestions-refresh, np-generate-plan.
    ...
    - **Smart Suggestions on Form**: Shows after photo selection with deterministic suggestions from POST /api/projects/:id/suggestions. Features inline loading ("Analyzing your photo…"), 4-5 checkmarked bullets, optional tags, and refresh button. Never blocks form interaction. Caption under photo: "We'll upload your photo later when you request a visual preview."
    - **Editable Build Prompt**: After suggestions load, users see a pre-filled prompt based on their details. TextInput allows full customization before generating the plan. Includes hint: "You get 1 visual preview per project. Refine with suggestions before generating your plan."
    - **Generate Plan**: Calls POST /api/projects/:id/build-without-preview with `{user_id: 'auto', prompt}`. Handles `{ok:false}` responses with full error message display. Shows loading state ("Generating…"), then navigates to Projects list on success.
